...
def new_docs(image):
    orig = image.copy()
    # <---- RESIZING -----> #
    image = functions.standard_resize(image, new_width = 100.0)
    # <---- RESIZING -----> #
    edged = functions.colorOps(image)
...
def prob_hough(image):
    ...
    functions.plot_images([orig, edged, rotated], ["Original", "Edged", "Rotated"])
...
def vanilla_box_drawing(image, drawing_image):
    boxes = functions.box_generation(image)
    ...
    boxes = functions.box_filtration(boxes, image.shape[1], image.shape[0])
    box = []
...
def boxes_from_edged(image, edging_function, **edging_args):
    ...
    closed_invert = functions.closed_inversion(edged) # perform closed inversion (morphological closing + color inversion) on the edged image
    boxes = functions.box_generation(closed_invert) # generate bounding boxes from the closed inversion output
    boxes = functions.box_filtration(boxes, image.shape[1], image.shape[0]) # filter bounding boxes
    merged = functions.merge_boxes(boxes)
...
def detection(image):
    ...
    merged, boxes, edged, closed_invert = boxes_from_edged(downsized, edging_function = functions.orig_page_edging)
    ...
    functions.plot_images([edged, closed_invert], titles = ["First pass", "First pass"])
    detected = image.copy()
    ...
    if not type(merged) == np.ndarray:
        ...
        merged, boxes, edged, closed_invert = boxes_from_edged(downsized, edging_function = functions.page_edging, thresh1=0, thresh2=160)
        functions.plot_images([edged, closed_invert], titles = ["Second pass", "Second pass"])
        # if not type(boxes) == np.ndarray or not boxes:
        if not type(merged) == np.ndarray:
            ...
            merged, boxes, edged, closed_invert = boxes_from_edged(downsized, edging_function = functions.page_edging, thresh1=0, thresh2=120)
            functions.plot_images([edged, closed_invert], titles = ["Third pass", "Third pass"])
    ...
    if type(merged) == np.ndarray:
            ...
            all_boxes = cv2.drawContours(downsized.copy(), contours = boxes, contourIdx = -1, color = (0,255,0), thickness = 1) # if just drawing one (using merge boxes)
    else: 
        print("FOUND NO BOXES AT ALL!!!!")
        functions.plot_images([original, edged, closed_invert, detected], titles = ["Original", "Edged", "Closed inversion", "Detected"])
        return
    ...
    downsize_cropped = functions.standard_resize(image =cropped, new_width = 100.0, return_ratio = False)
    ...
    functions.plot_images([original, closed_invert, all_boxes, cropped], titles = ["Original", "Close + Invert", "All boxes", "Cropped"])
    # return detected


def standard_hough(image):
    orig = image.copy()
    downsized, ratio = functions.standard_resize(image)
    # edged = functions.page_edging(downsized.copy())
    edged = functions.small_page_edging(downsized.copy())
    result = hough.standard_hough(edged, downsized)
    lines = hough.standard_hough(edged)
    corners = hough.hough_corners(lines)
    cornered = hough.draw_corners(orig, corners)
